...
Expression            union_exprssion_type;
}
...
%type <union_gpl_type> primary_expression
...
simple_type  T_ID  T_LBRACKET expression T_RBRACKET
{
 ...
 string id = *$2;
     ...
     if (sym_table->lookup(id))//if id is not in the symbol table
     {
      //assert($4 == INT);
//      int array_size = $4->eval_int();
       ...
       /*for (int i = 0; i < $4; i++)
       {
        ...
        Symbol * sym = new Symbol();
        if ($1 == INT)
        (*sym).set(name.str(), "INT", 0, 0, "0");
        if ($1 == DOUBLE)
        (*sym).set(name.str(), "DOUBLE", 0, 0, "0");
        if ($1 == STRING)
        (*sym).set(name.str(), "STRING", 0, 0, "0");
...
//     $$ = new Expression(variable);
    ...
    | T_INT_CONSTANT
    {
//     $$ = new Expression(INT, T_INT_CONSTANT);
     $$ = new Expression(INT, T_INT_CONSTANT);
    }
    | T_TRUE
    {
