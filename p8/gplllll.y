...
{
      Event_manager *e_man = Event_manager::instance();
      e_man->insert($2, Window::INITIALIZE);
}
...
{
      Gpl_type type_of_variable1 = $1->get_type();
      Gpl_type type_of_variable2 = $3->get_type();
      if (type_of_variable1 != GAME_OBJECT)
         assert(false);
      if (type_of_variable2 != GAME_OBJECT)
         assert(false);
      Expression * exp1 = new Expression($1);
      Expression * exp2 = new Expression($3);
      $$ = new Expression($2, exp1, exp2);
}
    ;
