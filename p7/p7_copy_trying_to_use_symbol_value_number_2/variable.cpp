...
void Variable::set_assignment_value(string action, Expression * exp)
{
  string name = get_name_for_assign_statement();
  Symbol_table *sym_table = Symbol_table::instance();
  Symbol * s = sym_table->get(name);
  Gpl_type type = s->get_type();
  if (return_string_type()!="member" && return_string_type()!= "member array" && return_string_type()!="member with variable index")
  {
    if (action == "=")
      set_equal(s, name, type, exp);
    /*
    if (action == "+")
      set_plus(s, name, type, exp);
    if (action == "-")
      set_minus(s, name, type, exp);
      */
  }
  /*
  else
  {
    ...
    if (action == "=")
      set_equal_object(s, name, type, exp, temp_object);
    if (action == "+")
      set_plus_object(s, name, type, exp, temp_object);
    if (action == "-")
      set_minus_object(s, name, type, exp, temp_object);
      }
      */
}
void Variable::set_equal(Symbol * s, string name, Gpl_type type, Expression * e)
{
    if (type == 1)
    {
      if (e->get_type() == 1)
      {
        s->new_assign_value(e->eval_int());
      }
      else if (e->get_type() == 2)
      {
        s->new_assign_value(e->eval_double());
      }
    }
    else if (type == 2)
    {
    ...
    else if (type == 4)
    {
      ...
      {
        s->new_assign_value(e->eval_string());
      }
    }
}
